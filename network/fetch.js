...
let Fetch = (url, option = {}) => {
  ...
  // 允许跨域
  // option.headers['Access-Control-Allow-Origin'] = option.headers['Access-Control-Allow-Origin'] || '*'
  // console.log("跨域" + option.headers['Access-Control-Allow-Origin'])
  // 获取 token
  option.headers['token'] = `${window.localStorage.getItem('token')}`;
  const m = (option.method || '').toLocaleLowerCase();
  // get query format
  if (m == 'get' && option.query) {
